...
(function (Apriori) {
    ...
    var AssociationRule = (function () {
        function AssociationRule(lhs, rhs, confidence,support,lift,opSubsetSupport) {
            this.lhs = lhs;
            this.rhs = rhs;
            this.confidence = confidence;
            this.support = support;
            this.lift = lift;
            this.rhsSupport = opSubsetSupport
        }
        ...
        Algorithm.prototype.analyze = function (transactions) {
            ...
            var saveAssociationRuleIfFound = function (subsetItemSet) {
                ...
                if (diffItemSet.length > 0) {
                    ...
                    // var opSubset = subsetItemSet.filter(function(value){
                    //     if(JSON.stringify(value)==JSON.stringify(diffItemSet)){
                    //         return false
                    //     }
                    //     return true;
                    // })
                    var opSubsetSupport = calculateSupport(diffItemSet,frequencies,transactions);
                    var lift = confidence/opSubsetSupport;
                    if (!isNaN(confidence) && !isTheRuleAlreadyFound(subsetItemSet) && confidence >= self.minConfidence) {
                        foundSubSets.push(subsetItemSet);
                        associationRules.push(new Apriori.AssociationRule(subsetItemSet, diffItemSet, confidence,subsetSupport,lift,opSubsetSupport));
                    }
                }
            };
